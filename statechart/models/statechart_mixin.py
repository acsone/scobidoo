...
class StatechartMixin(models.AbstractModel):
    ...
    def _compute_sc_has_allowed_events(self):
        sc_fields = self._get_sc_fields()
        if not self._sc_has_allowed_events:
            self.update({"sc_has_allowed_events": False})
        else:
            for rec in self:
